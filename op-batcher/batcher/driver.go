...
func (l *BatchSubmitter) publishToAltDAAndStoreCommitment(txdata txData, daGroup *errgroup.Group) {
	...
	// TODO: prob want to not pull the data out like before when altda has reached max # of goroutines...
	// in that case we could prob just use a blocking daGroup.Go here?
	daGroup.Go(func() error {
		...
		l.state.SaveAltDABlobCommitment(txdata, comm)
		return nil
	})
}
